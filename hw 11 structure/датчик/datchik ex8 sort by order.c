/*
Шаг 8: сортировка по запросу

Мы добавим интерактивное меню, которое будет предлагать пользователю выбор действий: 
  - сортировка по температуре (клавиша 'T')
  - сортировка по дате (клавиша 'D')
  - выход (клавиша 'E')

 При этом после каждой операции (кроме выхода) программа снова запрашивает действие.
 
  Изменения в main:
  1. После ввода данных (ручной, автоматический или файловый) и первоначального вывода (before sort) мы входим в цикл.
  2. В цикле выводим меню с запросом действия.
  3. В зависимости от введенного символа выполняем действие:
        'T' - сортировка по температуре и вывод
        'D' - сортировка по дате и вывод
        'E' - выход из цикла (завершение программы)
  4. Если введен неверный символ, выводим сообщение и повторяем запрос.
