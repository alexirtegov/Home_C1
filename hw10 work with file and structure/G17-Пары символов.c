/*
for G17-Пары символов

В файле input.txt записаны символы. Необходимо разработать 
функцию, которая меняет местами пары соседних символов 
...
Решение (олимпиада):
Считать строку из файла input.txt с помощью функции fgets.

Пройтись по строке по буквам слева направо. 
При этом пропускать пробелы (не обращать на них внимания).

Менять местами пары: найти первый непробельный символ — это символ 1. 
Затем найти следующий непробельный символ — это символ 2. 
Поменять их местами (в самой строке).

Потом найти следующий непробельный символ — это символ 3, 
и после него — символ 4. Снова поменять их местами.

Повторять, пока не дойдем до конца строки.
Если остался последний непробельный символ без пары — его не меняем.

Записать полученную строку в файл output.txt 
с помощью функции fputs или fprintf.
 */
